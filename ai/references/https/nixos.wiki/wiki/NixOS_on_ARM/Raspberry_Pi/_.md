...
* [Community](#) 
  + [About](/wiki/Nix_Community)
  + [Get In Touch](/wiki/Get_In_Touch)
  + [Support](/wiki/Support)
* [Wiki](#) 
  + [Contribute](/wiki/Contributing_to_the_wiki)
  + [Issues](https://github.com/nixos-users/wiki/issues)
  + [Projects](https://github.com/nixos-users/wiki/projects)
  + [Recent changes](/wiki/Special:RecentChanges)
  + [Random page](/wiki/Special:Random)
  + [Help](https://www.mediawiki.org/wiki/Special:MyLanguage/Help:Contents)

* [Log in](#) 
  + [With a wiki account](/index.php?title=Special:UserLogin&returnto=NixOS+on+ARM%2FRaspberry+Pi "You are encouraged to log in; however, it is not mandatory [o]")
  + [Login with GitHub](/index.php?title=Special:OAuth2Client/redirect&returnto=NixOS+on+ARM%2FRaspberry+Pi)
...
[← Back to NixOS on ARM](/wiki/NixOS_on_ARM "NixOS on ARM")

| Raspberry Pi Family | |
| --- | --- |
| [A Raspberry Pi 3 with enclosure.](/wiki/File:Raspberry_pi_3_glamour.jpg "A Raspberry Pi 3 with enclosure.") | |
| Raspberry Pi | |
| Architecture | ARMv6 |
| Raspberry Pi 2 | |
| Architecture | ARMv7 |
...
* [1 Status](#Status)
* [2 Board-specific installation notes](#Board-specific_installation_notes)
  + [2.1 Raspberry Pi (1)](#Raspberry_Pi_.281.29)
  + [2.2 Raspberry Pi 2](#Raspberry_Pi_2)
  + [2.3 Raspberry Pi 3 / 3B+](#Raspberry_Pi_3_.2F_3B.2B)
    - [2.3.1 HDMI output issue with kernel 5.4 (NixOS 20.03 or NixOS unstable)](#HDMI_output_issue_with_kernel_5.4_.28NixOS_20.03_or_NixOS_unstable.29)
  + [2.4 Raspberry Pi 4](#Raspberry_Pi_4)
  + [2.5 Audio](#Audio)
* [3 Serial console](#Serial_console)
* [4 Bluetooth](#Bluetooth)
* [5 Camera](#Camera)
* [6 Binary Cache](#Binary_Cache)
* [7 Kernel selection](#Kernel_selection)
* [8 Notes about the boot process](#Notes_about_the_boot_process)
  + [8.1 Boot option 1: boot.loader.generic-extlinux-compatible](#Boot_option_1:_boot.loader.generic-extlinux-compatible)
  + [8.2 Boot option 2: boot.loader.raspberryPi](#Boot_option_2:_boot.loader.raspberryPi)
  + [8.3 Raspberry Pi (all versions)](#Raspberry_Pi_.28all_versions.29)
* [9 Device trees](#Device_trees)
* [10 Troubleshooting](#Troubleshooting)
  + [10.1 Power issues](#Power_issues)
  + [10.2 WiFi / WLAN](#WiFi_.2F_WLAN)
  + [10.3 HDMI](#HDMI)
    - [10.3.1 Early boot messages](#Early_boot_messages)
  + [10.4 Raspberry Pi 3B+ HDMI output issues](#Raspberry_Pi_3B.2B_HDMI_output_issues)
  + [10.5 Additional Troubleshooting](#Additional_Troubleshooting)
...
The following table is intended to be updated by the NixOS contributors with the current status of the boards. For a list of products, [see the *Products Archive*](https://www.raspberrypi.org/products/).
...
| Raspberry Pi 3 | | |
| Raspberry Pi 3 Model B | AArch64  *+ armv7* | YES |
| Raspberry Pi 3 Model B+ | YES |
| Raspberry Pi 3 Model A+ | ? |
| Raspberry Pi 4 | | |
| Raspberry Pi 4 Model B | AArch64  *+ armv7* | YES |
| Raspberry Pi 400 | | |
| Raspberry Pi 400 | AArch64  *+ armv7* | Yes (only with kernel >= 6.1) |
...
First follow the [generic installation steps](/wiki/NixOS_on_ARM#Installation "NixOS on ARM") to get the installer image and install using the [installation and configuration steps](/wiki/NixOS_on_ARM#NixOS_installation_.26_configuration "NixOS on ARM").
...
**Note:** The mainline kernel (tested with nixos kernel 4.18.7) [does not include support for cpu frequency scaling](https://github.com/raspberrypi/linux/wiki/Upstreaming#downstream-drivers) on the Raspberry Pi. To get higher clock speed, set [`force_turbo=1`](https://www.raspberrypi.org/documentation/configuration/config-txt/overclocking.md) in `/boot/config.txt`
...
Some users have reported that the 5.4 kernel "hung at Starting kernel..." [![Issue.png](/images/3/3c/Issue.png)](https://github.com/NixOS/nixpkgs/issues/82455)[#82455](https://github.com/NixOS/nixpkgs/issues/82455). In all cases where it was possible to investigate, it was found that the device did boot, but that the HDMI out didn't function as expected.

It looks like it may be a setup-dependent issue, as a 20.03 image with 5.4 was verified as working.

If your setup is having the issue, first report on [![Issue.png](/images/3/3c/Issue.png)](https://github.com/NixOS/nixpkgs/issues/82455)[#82455](https://github.com/NixOS/nixpkgs/issues/82455) with the Raspberry Pi model (important to note whether it is a plus or non-plus) and the kind of display used with the HDMI out, including whether it is using adapters or not.
...
*See* [NixOS on ARM/Raspberry Pi 4](/wiki/NixOS_on_ARM/Raspberry_Pi_4 "NixOS on ARM/Raspberry Pi 4")

### Audio

In addition to the usual config, it might be required to enable audio support explicitly in the firmwareConfig.

![Breeze-text-x-plain.png](/images/e/ef/Breeze-text-x-plain.png)
...
Depending on the architecture used, binary caches availability varies. Binary caches instructions are on the main [NixOS on ARM](/wiki/NixOS_on_ARM#Binary_cache "NixOS on ARM") page. The following table describes the architectures supported by each board.
...
Linux 4.14) have a hard time dealing with it properly. If the power supply is rated properly AND the cable is not incurring too much power losses, it may be required to disable the lightning bolt indicator so the display driver isn't messed up.[[1]](#cite_note-1) The lightning bolt indicator can be disabled by adding the line `avoid_warnings=1` in config.txt[[2]](#cite_note-2)
...
For now, do not use linuxPackages\_latest, use the default linuxPackages which is the latest LTS, 4.19, which is known to be compatible.

See [![Issue.png](/images/3/3c/Issue.png)](https://github.com/NixOS/nixpkgs/issues/66960)[#66960](https://github.com/NixOS/nixpkgs/issues/66960).
...
1. [↑](#cite_ref-1) <https://logs.nix.samueldr.com/nixos/2017-12-20#1513784657-1513784714>;
2. [↑](#cite_ref-2) <https://www.raspberrypi.org/documentation/configuration/config-txt/README.md>

Retrieved from "<https://nixos.wiki/index.php?title=NixOS_on_ARM/Raspberry_Pi&oldid=13331>"

[Category](/wiki/Special:Categories "Special:Categories"):

* [Pages with syntax highlighting errors](/index.php?title=Category:Pages_with_syntax_highlighting_errors&action=edit&redlink=1 "Category:Pages with syntax highlighting errors (page does not exist)")



[Discussion](/wiki/Talk:NixOS_on_ARM/Raspberry_Pi "Discussion about the content page [t]")
...
* [Log in](/index.php?title=Special:UserLogin&returnto=NixOS+on+ARM%2FRaspberry+Pi "You are encouraged to log in; however, it is not mandatory [o]")
* [Create account](/index.php?title=Special:CreateAccount&returnto=NixOS+on+ARM%2FRaspberry+Pi "You are encouraged to create an account and log in; however, it is not mandatory")

[View source](/index.php?title=NixOS_on_ARM/Raspberry_Pi&action=edit "This page is protected.
